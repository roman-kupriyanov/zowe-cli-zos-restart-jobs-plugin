...
describe("RestartJobs tests", () => {
    ...
    const job: IJob = {
        ...
        class: "A",
        "files-url":
            "https://127.0.0.1:443/zosmf/restjobs/jobs/J0004541SVSCJES2D74CB05A.......%3A/files",
        jobname: "TESTJOB",
        status: "OUTPUT",
        retcode: "ABEND S806",
    };

    const restartJobJcl: string =
        "//TESTJOB JOB (ACCTINFO),'user',CLASS=A,\n" +
        "//             RESTART=(STEP02),\n" +
        "//STEP01   EXEC PGM=XYZ\n" +
    ...
    describe("getRestartJclForJob tests", () => {
        ...
        it("should success with single line JOB statement", async () => {
            const jobJcl: string =
                "//TESTJOB JOB (ACCTINFO),'user',CLASS=A                                JOB04541\n" +
                "//STEP01   EXEC PGM=XYZ\n" +
                "//STEP02   EXEC PGM=IEFBR14";

            const modifiedJobJcl: string =
                "//TESTJOB JOB (ACCTINFO),'user',CLASS=A,\n" +
                "// RESTART=(STEP02)\n" +
                "//STEP01   EXEC PGM=XYZ\n" +
                "//STEP02   EXEC PGM=IEFBR14";
            ...
            const resultJobJcl: string = await RestartJobs.getRestartJclForJob(
                dummySession,
                stepname,
                job
            );
        ...
        it("should success with multi line JOB statement", async () => {
            const jobJcl: string =
                "//TESTJOB JOB (ACCTINFO),'user',                                       JOB04541\n" +
                "//             CLASS=A\n" +
                "//STEP01   EXEC PGM=XYZ\n" +
                "//STEP02   EXEC PGM=IEFBR14";

            const modifiedJobJcl: string =
                "//TESTJOB JOB (ACCTINFO),'user',\n" +
                "//             CLASS=A,\n" +
                "// RESTART=(STEP02)\n" +
                "//STEP01   EXEC PGM=XYZ\n" +
                "//STEP02   EXEC PGM=IEFBR14";
        ...
        it("should success with single line JOB statement and RESTART= already specified there", async () => {
            const jobJcl: string =
                "//TESTJOB JOB (ACCTINFO),'user',RESTART=(STEP01)                       JOB04541\n" +
                "//STEP01   EXEC PGM=XYZ\n" +
                "//STEP02   EXEC PGM=IEFBR14";

            const modifiedJobJcl: string =
                "//TESTJOB JOB (ACCTINFO),'user',RESTART=(STEP02)\n" +
                "//STEP01   EXEC PGM=XYZ\n" +
                "//STEP02   EXEC PGM=IEFBR14";
        ...
        it("should error when no step name found in jcl", async () => {
            ...
            try {
                await RestartJobs.getRestartJclForJob(
                    dummySession,
                    "STEP03",
                    job as IJob
                );
    ...
    describe("getFailedJobRestartJcl tests", () => {
        ...
        const getRestartJclForJobSpy = jest.spyOn(
            RestartJobs,
            "getRestartJclForJob"
        );
        ...
        it("should success", async () => {
            ...
            getRestartJclForJobSpy.mockImplementation(() =>
                Promise.resolve(restartJobJcl)
            );

            const resultJobJcl: string =
                await RestartJobs.getFailedJobRestartJcl(
                    dummySession,
                    jobid,
                    stepname
                );
            ...
            expect(getRestartJclForJobSpy).toHaveBeenCalledWith(
                dummySession,
                stepname,
                job
            );
    ...
    describe("restartFailedJob tests", () => {
        const getFailedJobRestartJclSpy = jest.spyOn(
            RestartJobs,
            "getFailedJobRestartJcl"
        );
        ...
        it("should success", async () => {
            getFailedJobRestartJclSpy.mockImplementation(() =>
                Promise.resolve(restartJobJcl)
            );
            submitJclSpy.mockImplementation(() => Promise.resolve(restartJob));

            const resultJob = await RestartJobs.restartFailedJob(
                dummySession,
                jobid,
                stepname
            );

            expect(getFailedJobRestartJclSpy).toHaveBeenCalledWith(
                dummySession,
                jobid,
                stepname
            );
            expect(submitJclSpy).toHaveBeenCalledWith(
                dummySession,
                restartJobJcl
            );
    ...
    describe("restartFailedJobWithParms tests", () => {
        ...
        it("should success with no wait for output", async () => {
            ...
            submitJclStringSpy.mockImplementation(() =>
                Promise.resolve(restartJob)
            );

            const resultJob = await RestartJobs.restartFailedJobWithParms(
                dummySession,
                jobid,
                stepname,
                restartParms
            ...
            expect(submitJclStringSpy).toHaveBeenCalledWith(
                dummySession,
                restartJobJcl,
                submitParms
            );
